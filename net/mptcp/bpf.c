					  ...
					  int off, int size)
{
	...
	if (id == mptcp_sock_id) {
		...
		case offsetof(struct mptcp_sock, pm.local_addr_used):
			...
			break;
		case offsetof(struct mptcp_sock, pm.extra_subflows):
			end = offsetofend(struct mptcp_sock, pm.extra_subflows);
			break;
		default:
...
static bool __bpf_mptcp_pm_accept_new_subflow(struct mptcp_sock *msk, bool allow)
{
	return false;
}
...
static void __bpf_mptcp_pm_add_addr_received(struct mptcp_sock *msk)
{
}

static void __bpf_mptcp_pm_rm_addr_received(struct mptcp_sock *msk, u8 id)
{
}

static bool __bpf_mptcp_pm_accept_new_address(struct mptcp_sock *msk,
