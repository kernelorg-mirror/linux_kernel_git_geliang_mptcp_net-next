...
static void test_bpf_netlink_pm(void)
{
	...
	err = err ?: bpf_program__set_flags(skel->progs.mptcp_pm_netlink_subflow_established,
					    BPF_F_SLEEPABLE);
	err = err ?: bpf_program__set_flags(skel->progs.mptcp_pm_netlink_add_addr_received,
					    BPF_F_SLEEPABLE);
...
static void test_bpf_userspace_pm(void)
{
	...
	err = bpf_program__set_flags(skel->progs.mptcp_pm_userspace_address_announce,
				     BPF_F_SLEEPABLE);
	err = err ?: bpf_program__set_flags(skel->progs.mptcp_pm_userspace_address_remove,
					    BPF_F_SLEEPABLE);
	err = err ?: bpf_program__set_flags(skel->progs.mptcp_pm_userspace_subflow_create,
					    BPF_F_SLEEPABLE);
	err = err ?: bpf_program__set_flags(skel->progs.mptcp_pm_userspace_subflow_destroy,
					    BPF_F_SLEEPABLE);
...
static void run_iters_netlink_address(void)
{
	int server_fd, client_fd, accept_fd;
	int is_mptcp, err;
	socklen_t len;
	...
	err = netlink_pm_add_subflow(ADDR_2, 20);
	err = err ?: netlink_pm_add_subflow(ADDR_3, 30);
	err = err ?: netlink_pm_add_subflow(ADDR_4, 40);
	if (!ASSERT_OK(err, "netlink_pm_add_subflow"))
		goto close_accept;
...
static void test_iters_netlink_address(void)
{
	struct mptcp_bpf_iters *skel;
	struct netns_obj *netns;
	int cgroup_fd;
	...
	cgroup_fd = test__join_cgroup("/iters_nl_address");
	if (!ASSERT_OK_FD(cgroup_fd, "join_cgroup: iters_nl_address"))
		return;

	skel = mptcp_bpf_iters__open_and_load();
	if (!ASSERT_OK_PTR(skel, "skel_open_load: iters_nl_address"))
		goto close_cgroup;

	skel->links.netlink_addr = bpf_program__attach_cgroup(skel->progs.netlink_addr,
							      cgroup_fd);
	if (!ASSERT_OK_PTR(skel->links.netlink_addr, "attach getsockopt"))
		goto skel_destroy;
	...
	err = pm_init("kernel");
	if (!ASSERT_OK(err, "pm_init: iters_nl_address"))
		goto close_netns;

	run_iters_netlink_address();

	/* 20 + 30 + 40 = 90 */
	ASSERT_EQ(skel->bss->ids, 90, "nl address ids");
