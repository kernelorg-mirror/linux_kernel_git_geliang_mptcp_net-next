...
do_tcpmd5_tests()
{
	...
	vrf=$(ip netns exec "$ns_sbox" ip link show vrf-test |
				       awk -F':' '{print $1; exit}')
	do_tcpmd5_test -m "md5ext,prefixlen=1,ifindex=$vrf,key=9876543210"
	lret=$?
	if [ $lret -ne 0 ] ; then
		ip netns exec "$ns_sbox" ip link del vrf-test
		return $lret
	fi
	do_tcpmd5_test -6 -m "md5ext,prefixlen=1,ifindex=$vrf,key=9876543210"
	lret=$?
